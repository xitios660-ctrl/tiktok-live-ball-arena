...
function rateLimit(req: { ip?: string }, res: any, next: () => void): void {
  ...
  else { old.count += 1; if (old.count > 40) { res.status(429).json({ ok: false, error: 'Muitas tentativas. Aguarde um minuto.' }); return; } }
  next();
}
...
export function shopApiRouter(store: EconomyStore): Router {
  const r = Router(); r.use(rateLimit);
